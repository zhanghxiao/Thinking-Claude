class CodeBlockCollapser {
    ...
    createCopyButton() {
        ...
        container.className = 'from-bg-300/90 to-bg-300/70 pointer-events-auto rounded-md bg-gradient-to-b p-0.5 backdrop-blur-md';
        
        const button = document.createElement('button');
        button.className = 'flex flex-row items-center gap-1 rounded-md p-1 py-0.5 text-xs transition-opacity delay-100 hover:bg-bg-200 opacity-60 hover:opacity-100';
        ...
        button.addEventListener('click', async () => {
            const codeText = button.closest('pre')?.querySelector('code')?.textContent;
            if (codeText) {
    ...
    processExistingBlocks() {
        document.querySelectorAll('pre').forEach(pre => {
            ...
            if (header && header.textContent.trim() === 'thinking' && !pre.querySelector('.thinking-header')) {
    ...
    processBlock(pre) {
        ...
        if (mainContainer) {
            mainContainer.insertBefore(headerContainer, pre.querySelector('.code-block__code').parentElement);
